...
public class VOSClusteringTechnique
{
    ...
    public double calcQualityFunction()
    {
        ...
        for (i = 0; i < network.nNodes; i++)//sum(ki)
            clusterWeight[clustering.cluster[i]] += network.nodeWeight[i];
        for (i = 0; i < clustering.nClusters; i++)
            qualityFunction -= clusterWeight[i] * clusterWeight[i] * resolution;//resolution=1/2m
//maybe incorrect
    ...
    public boolean runLocalMovingAlgorithm(Random random)
    {
        ...
        nodePermutation = Arrays2.generateRandomPermutation(network.nNodes, random);//shuffle
        ...
        do
        {
            ...
            for (k = 0; k < nNeighboringClusters; k++)
            {
                ...
                qualityFunction = edgeWeightPerCluster[l] - network.nodeWeight[j] * clusterWeight[l] * resolution;//deltaQ
