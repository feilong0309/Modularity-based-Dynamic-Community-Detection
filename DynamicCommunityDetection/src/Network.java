...
*/
    public Network(int nNodes, int[] firstNeighborIndex, int[] neighbor, double[] edgeWeight, HashSet<Integer> hs)
    {
        this(nNodes, null, firstNeighborIndex, neighbor, edgeWeight, hs);
    }

    public Network(int nNodes, double[] nodeWeight, int[] firstNeighborIndex, int[] neighbor, double[] edgeWeight, HashSet<Integer> hs)
    {
        this.nNodes = nNodes;
        this.hs = new HashSet<Integer>(hs);
    ...
    public Network combineNetwork(Network network,int sn,int en, double wt){
    	int i,node1,node2;
    	double weight12;
    	boolean doExist;
    	int weightChangeIndex=-1;
    	double originEdgeWeight1=-1000;
    	double originEdgeWeight2=-1000;
    	Network combine=new Network(network.nNodes, network.firstNeighborIndex, network.neighbor, network.edgeWeight, network.hs);
//    	node1=newNetwork.neighbor[0]>newNetwork.neighbor[1]?newNetwork.neighbor[1]:newNetwork.neighbor[0];				
//		node2=newNetwork.neighbor[1]>newNetwork.neighbor[0]?newNetwork.neighbor[1]:newNetwork.neighbor[0];
//		weight12=newNetwork.nodeWeight[node1];
    	int[] pieceEdge;
    	double[] pieceEdgeWeight;
    	node1=sn;
		node2=en;
		weight12=wt;
...
//		combine.nEdges+=2;
//		combine.edgeWeight=new double[combine.nEdges];
//		combine.neighbor= new int[combine.nEdges];
//		combine.hs.add(node1);
//		combine.hs.add(node2);
		doExist=false;
		if(node1<network.nNodes && node2<network.nNodes){
			pieceEdge = Arrays.copyOfRange(network.neighbor,network.firstNeighborIndex[node1],network.firstNeighborIndex[node1+1]);
			pieceEdgeWeight= Arrays.copyOfRange(network.edgeWeight,network.firstNeighborIndex[node1],network.firstNeighborIndex[node1+1]);

			for(i=0;i<pieceEdge.length;i++){
				if(pieceEdge[i]==node2){
					doExist=true;
					weightChangeIndex=i;
					originEdgeWeight1=pieceEdgeWeight[i];
					break;
				}
			}
			if(doExist==true){
//				System.out.println("pieceEdgeWeight[weightChangeIndex]"+pieceEdgeWeight[weightChangeIndex]);
			}
		}
		if(doExist==true){
			int[] pieceEdge2 = Arrays.copyOfRange(network.neighbor,network.firstNeighborIndex[node2],network.firstNeighborIndex[node2+1]);
			double[] pieceEdgeWeight2= Arrays.copyOfRange(network.edgeWeight,network.firstNeighborIndex[node2],network.firstNeighborIndex[node2+1]);
			int weightChangeIndex2=-1;
			for(i=0;i<pieceEdge2.length;i++){
				if(pieceEdge2[i]==node1){
					weightChangeIndex2=i;
					originEdgeWeight2=pieceEdgeWeight2[i];
					break;
				}
			}
//			System.out.println("network.firstNeighborIndex[node1]+weightChangeIndex"+network.firstNeighborIndex[node1]+weightChangeIndex);
			combine.edgeWeight[network.firstNeighborIndex[node1]+weightChangeIndex]=weight12;
			combine.edgeWeight[network.firstNeighborIndex[node2]+weightChangeIndex2]=weight12;
			combine.nodeWeight[node1]+=(-originEdgeWeight1+weight12);
			combine.nodeWeight[node2]+=(-originEdgeWeight2+weight12);
			
//			combine.nEdges=network.nEdges;
//			combine.nNodes=network.nNodes;
//			for(i=0;i<network.nNodes;i++){
//				combine.firstNeighborIndex[i]=combine.firstNeighborIndex[i];
//			}
//			for(i=0;i<network.nEdges;i++){
//				combine.edgeWeight[i]=combine.edgeWeight[i];
//			}
			...
			combine.hs.add(node1);
			combine.hs.add(node2);
			if(node1<network.nNodes && node2<network.nNodes){//node 1 and node 2 both exist in the original network
				for(i=0;i<network.nNodes+1;i++){
					combine.firstNeighborIndex[i]=(i<=node1?network.firstNeighborIndex[i]:(i<=node2?network.firstNeighborIndex[i]+1:network.firstNeighborIndex[i]+2));
					if(i<network.nNodes){
						combine.nodeWeight[i]=((i==node1 || i==node2)?network.nodeWeight[i]+weight12:network.nodeWeight[i]);
					}
				}
				for(i=0;i<combine.nEdges;i++){//update edgeWeight and neighbor
					if(i<network.firstNeighborIndex[node1]){
						combine.edgeWeight[i]=network.edgeWeight[i];
						combine.neighbor[i]=network.neighbor[i];
					}else if(i==network.firstNeighborIndex[node1]){
						combine.edgeWeight[i]=weight12;
						combine.neighbor[i]=node2;
					}else if(i<network.firstNeighborIndex[node2]+1){
						combine.edgeWeight[i]=network.edgeWeight[i-1];
						combine.neighbor[i]=network.neighbor[i-1];
					}else if(i==network.firstNeighborIndex[node2]+1){
						combine.edgeWeight[i]=weight12;
						combine.neighbor[i]=node1;
					}else{
						combine.edgeWeight[i]=network.edgeWeight[i-2];
						combine.neighbor[i]=network.neighbor[i-2];
					}			
				}
			}else if(node1<network.nNodes && !(node2<network.nNodes)){//node 1 exist in the original network, node2 is new node
				combine.nNodes=network.nNodes+1;
				combine.firstNeighborIndex = new int[network.nNodes+2];
				combine.nodeWeight = new double[network.nNodes+1];
				for(i=0;i<network.nNodes+1;i++){
					combine.firstNeighborIndex[i]=(i<=node1?network.firstNeighborIndex[i]:network.firstNeighborIndex[i]+1);
					if(i<network.nNodes){
						combine.nodeWeight[i]=(i==node1?network.nodeWeight[i]+weight12:network.nodeWeight[i]);
					}
				}
				combine.firstNeighborIndex[combine.nNodes]=network.firstNeighborIndex[network.nNodes]+2;
				combine.nodeWeight[network.nNodes]=weight12;
				for(i=0;i<combine.nEdges;i++){//update edgeWeight and neighbor
					if(i<network.firstNeighborIndex[node1]){
						combine.edgeWeight[i]=network.edgeWeight[i];
						combine.neighbor[i]=network.neighbor[i];
			...
			}else if(node1>=network.nNodes){
				combine.nNodes=network.nNodes+2;
				combine.firstNeighborIndex = new int[network.nNodes+3];
				combine.nodeWeight = new double[network.nNodes+2];	
				for(i=0;i<network.nNodes+1;i++){
					combine.firstNeighborIndex[i]=network.firstNeighborIndex[i];
					if(i<network.nNodes){
						combine.nodeWeight[i]=network.nodeWeight[i];
					}
				}
				combine.firstNeighborIndex[combine.nNodes-1]=network.firstNeighborIndex[network.nNodes]+1;
				combine.firstNeighborIndex[combine.nNodes]=network.firstNeighborIndex[network.nNodes]+2;
				combine.nodeWeight[network.nNodes]=weight12;
				combine.nodeWeight[network.nNodes+1]=weight12;
				for(i=0;i<combine.nEdges;i++){//update edgeWeight and neighbor
					if(i<network.firstNeighborIndex[node1]){
						combine.edgeWeight[i]=network.edgeWeight[i];
						combine.neighbor[i]=network.neighbor[i];
